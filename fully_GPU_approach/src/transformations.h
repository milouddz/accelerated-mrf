...
__global__ void filter1_gpu(float* imagein,float* imageout,int m,int n,int o,float* filterr,int length,int dim){
    
    float filter[3]={-0.5,0.0,0.5};
    int f;
    int hw=(length-1)/2;
...
__global__ void jacobian_gpu_1(float*J11,float*J12,float*J13,float*J21,float*J22,float*J23,float*J31,float*J32,float*J33,float factor1){
...
__global__ void jacobian_gpu_2(float* J11,float* J22,float* J33){
...
__global__ void jacobian_gpu_3(float*J11,float*J12,float*J13,float*J21,float*J22,float*J23,float*J31,float*J32,float*J33,float* jmean,float* Jac,int sz){

    float J;
    float neg=0; float Jmin=1; float Jmax=1;
    int i = threadIdx.x + blockIdx.x * blockDim.x;

    J=J11[i]*(J22[i]*J33[i]-J23[i]*J32[i])-J21[i]*(J12[i]*J33[i]-J13[i]*J32[i])+J31[i]*(J12[i]*J23[i]-J13[i]*J22[i]);
    atomicAdd(jmean,J/sz);
    if(J>Jmax)
        Jmax=J;
    if(J<Jmin)
        Jmin=J;
...
__global__ void jacobian_gpu_4(float* Jac,float*jmean,float* jstd,int sz){

    int i = threadIdx.x + blockIdx.x*blockDim.x;
    
    atomicAdd(jstd,pow(Jac[i]-*jmean,2.0));
    
}
float jacobian_GPU(float* d_u1,float* d_v1,float* d_w1,int m,int n,int o,int factor){
    
    float factor1=1.0/(float)factor;
    float jmean=0.0;
    float* d_jmean;
    cudaMalloc((void**)&d_jmean,sizeof(float));

    float jstd=0.0;
    float* d_jstd;
    cudaMalloc((void**)&d_jstd,sizeof(float));
    cudaMemset(d_jstd,0.0,sizeof(float));
    ...
    int sz = m*n*o;
    float* d_Jac;
    cudaMalloc((void**)&d_Jac,sz*sizeof(float));
    ...
    cudaMalloc((void**)&d_J11,sz*sizeof(float));
    cudaMalloc((void**)&d_J12,sz*sizeof(float));
    cudaMalloc((void**)&d_J13,sz*sizeof(float));
    cudaMalloc((void**)&d_J21,sz*sizeof(float));
    cudaMalloc((void**)&d_J22,sz*sizeof(float));
    cudaMalloc((void**)&d_J23,sz*sizeof(float));
    cudaMalloc((void**)&d_J31,sz*sizeof(float));
    cudaMalloc((void**)&d_J32,sz*sizeof(float));
    cudaMalloc((void**)&d_J33,sz*sizeof(float));

    cudaMemset(d_J11,0.0,sz*sizeof(float));
    cudaMemset(d_J12,0.0,sz*sizeof(float));
    cudaMemset(d_J13,0.0,sz*sizeof(float));
    cudaMemset(d_J21,0.0,sz*sizeof(float));
    cudaMemset(d_J22,0.0,sz*sizeof(float));
    cudaMemset(d_J23,0.0,sz*sizeof(float));
    cudaMemset(d_J31,0.0,sz*sizeof(float));
    cudaMemset(d_J32,0.0,sz*sizeof(float));
    cudaMemset(d_J33,0.0,sz*sizeof(float));
    
    float neg=0; float Jmin=1; float Jmax=1; float J;
    float count=0; float frac;
    
    dim3 grid(m,n,o);
    filter1_gpu<<<grid,1>>>(d_u1,d_J11,m,n,o,grad,3,2);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_u1,d_J12,m,n,o,grad,3,1);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_u1,d_J13,m,n,o,grad,3,3);
    cudaDeviceSynchronize();
    
    filter1_gpu<<<grid,1>>>(d_v1,d_J21,m,n,o,grad,3,2);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_v1,d_J22,m,n,o,grad,3,1);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_v1,d_J23,m,n,o,grad,3,3);
    cudaDeviceSynchronize();

    filter1_gpu<<<grid,1>>>(d_w1,d_J31,m,n,o,grad,3,2);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_w1,d_J32,m,n,o,grad,3,1);
    cudaDeviceSynchronize();
    filter1_gpu<<<grid,1>>>(d_w1,d_J33,m,n,o,grad,3,3);
    cudaDeviceSynchronize();

    jacobian_gpu_1<<<grid,1>>>(d_J11,d_J12,d_J13,d_J21,d_J22,d_J23,d_J31,d_J32,d_J33,factor1);
    cudaDeviceSynchronize();
    
    jacobian_gpu_2<<<grid,1>>>(d_J11,d_J22,d_J33);
    cudaDeviceSynchronize();
    
    cudaMemset(d_jmean,0.0,sizeof(float));
    jacobian_gpu_3<<<sz,1>>>(d_J11,d_J12,d_J13,d_J21,d_J22,d_J23,d_J31,d_J32,d_J33,d_jmean,d_Jac,sz);
    cudaDeviceSynchronize();

    jacobian_gpu_4<<<sz,1>>>(d_Jac,d_jmean,d_jstd,sz);
    cudaDeviceSynchronize();
    cudaMemcpy(&jstd,d_jstd,sizeof(float),cudaMemcpyDeviceToHost);

    jstd/=(m*n*o-1);
    jstd=sqrt(jstd);
    frac=neg/count;
    cout<<"std(J)="<<std::round(jstd*100)/100.0;
    //cout<<"Range: ["<<Jmin<<", "<<Jmax<<"]round(jmean*100)/100.0<<
    cout<<" (J<0)="<<std::round(frac*1e7)/100.0<<"e-7  ";
    delete []Jac;
